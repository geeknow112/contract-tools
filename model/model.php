...
//var_dump($params);
            ...
            //DBの接続情報
            include('/home/bitnami/apps/wp-sales/htdocs/wp-content/plugins/mirai/db_config.php');

            //ユーザ単位のDB情報を取得
            include('/home/bitnami/apps/wp-sales/htdocs/wp-content/plugins/mirai/user_config.php');
            ...
            //ログインユーザによってテーブル名を切り替える
            $tablename = $usersData[$user_login]['table'];

            //入力された検索条件からSQl文を生成
            $where = [];

            if(!empty($params['InputDate'])){
                    $where[] = "order_dt >= '".$params['InputDate']."'";
            }

            if(!empty($params['InputDate2'])){
                    $where[] = "order_dt <= '".$params['InputDate2']."'";
            }

            if(!empty($params['OrderID'])){
                    $where[] = "order_id like '%{$params['OrderID']}%'";
            }

            if(!empty($params['Sales'])){
                    $where[] = "sales in (".$params['Sales'].")";
            }
...
//var_dump("api=".$params['api']."<br />");
/*
	    if($params['api'] == 1){
                    $sql = $sql ." group by sales,dest,detail order by sales,dest,detail asc";
	    }elseif($params['api'] == 2){
                    $sql = $sql ." group by sales,dest,ship_code order by sales,dest,ship_code asc";
	    }else{
                    $sql = $sql ." group by sales order by order_dt desc limit 30";
	    }
*/
                    $sql = $sql ." group by sales order by order_dt desc limit 30";
//print($sql);
...
/*
		 'shop' => array(
			...
			'validator' => array(
				array("notEmpty"),
				array("Regex", "/^[0-9a-zA-Z\-\_]+$/"),
				array("MbstringLength", 1, 16),
				new $exists('shop', 'shop', null, $this->getDbCon()),
//				 array("Db_RecordExists", array('shop', 'shop', null, $this->getDbCon())),
//				 new Zend_Validate_Db_RecordExists('shop', 'shop', quoteInto('shop = ?', $params['shop'])),
			),
		  ),
		// ショップURL
		 'shop_url' => array(
			'label' => 'ショップURL',
			'validator' => array(
				array("notEmpty"), array("AlnumEx"), array("MbstringLength", 1, 50)
			),
		  ),
		// ショップ名
		 'name' => array(
			'label' => 'ショップ名',
			'validator' => array(
				"allowEmpty" => true, array("notEmpty"), array("MbstringLength", 1, 100),
				array("Regex", "/^(?!.*[&\"\'<>]).*$/u"),
			),
		  ),
		// ショップ名カナ
