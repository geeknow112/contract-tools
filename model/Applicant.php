		...
		switch ($service_type) {
			...
			default :
				$sql  = "SELECT 
							...
							'PC' as site_type, 
							'https://www.sample.jp/' as other_site_url, 
							'準備中' as site_class, 
							'-' as release_dt, 
...
*/
	 ...
	 * 
	 **/
	private function getPartsService() {
		return array(
			'0' => '出店',
			'1' => 'クレジットカード決済サービスのお申込'
		);
	}
	 ...
	 * 
	 **/
	private function getPartsPlan() {
		return array(
			'0' => array('name' => 'ハイプラン', 'discription' => 'プラン内容'),
			'1' => array('name' => 'ロープラン', 'discription' => 'プラン内容'),
			'2' => array('name' => 'Aプラン', 'discription' => 'プラン内容'),
			'3' => array('name' => 'Bプラン', 'discription' => 'プラン内容'),
			'4' => array('name' => '入門', 'discription' => 'プラン内容')
		);
	}
	 ...
	 * 
	 **/
	private function getPartsBizFg() {
		return array(
			0 => '', 
			1 => '法人', 
			2 => '個人事業主'
	 ...
	 * 「商品以外の必要料金」
	 **/
	private function getPartsExpenses() {
		return array(
			'1' => '消費税', 
			'2' => '送料', 
			'9' => 'その他', 
	 ...
	 * 「不良品の取扱」
	 **/
	private function getPartsDefective() {
		return array(
			'1' => '送料当社負担にて返送', 
			'9' => 'その他', 
		);
	 ...
	 * 「販売数量」
	 **/
	private function getPartsSalesQty() {
		return array(
			'1' => '1個から', 
			'9' => 'その他', 
		);
	 ...
	 * 「支払方法」
	 **/
	private function getPartsPayment() {
		return array(
			'1' => 'クレジットカード', 
			'2' => '代引き', 
			'9' => 'その他', 
	 ...
	 * 「返品について」
	 **/
	private function getPartsAboutReturns() {
		return array(
			'1' => '使用後も可', 
			'2' => '使用前に限る', 
			'9' => 'その他', 
	 ...
	 * 「返品送料」
	 **/
	private function getPartsReturnShipping() {
		return array(
			'1' => '当社負担', 
			'2' => '購入者負担', 
			'9' => 'その他', 
	 ...
	 * 
	 **/
	private function getPartsGoodsClass() {
		return array(
			''  => '', 
			'1' => '食料品・飲料', 
			'2' => '酒', 
	...
	public function getAliases() {
		return array(
// STEP1
			'apply_service'				=> 'お申込サービス',
			'apply_plan'				=> 'お申込プラン',
			'biz_fg'					=> '事業形態',
			'biz_number'				=> '法人番号',
			'company_name'				=> '社名',
			'company_name_kana'			=> '社名（フリガナ）',
			'zip'						=> '郵便番号',
			'pref'						=> '都道府県',
			'addr'						=> '市区群',
			'addr2'						=> '町名番地',
			...
			'goods_class'				=> '取扱商品分類',
			'goods'						=> '取扱商材',
			'delivery_company'			=> '代引契約している配送会社',
			'url'						=> 'ホームページURL',
			'ceo_name_sei'				=> '代表者名：姓',
			'ceo_name_kana_sei'			=> '代表者名（フリガナ）：セイ',
			'ceo_name_mei'				=> '代表者名：名',
			'ceo_name_kana_mei'			=> '代表者名（フリガナ）：メイ',
			'ceo_birth'					=> '代表者生年月日',
			'ceo_addr_fg'				=> '代表者自宅住所',
			'ceo_zip'					=> '代表者自宅郵便番号',
			'ceo_pref'					=> '代表者自宅都道府県',
			...
			'ceo_addr2'					=> '代表者自宅町名番地',
			'ceo_addr3'					=> '代表者自宅ビル名',
			'ceo_addr_kana'				=> '代表者自宅住所（フリガナ）',
			'ceo_tel'					=> '代表者自宅電話番号',
			'corp_fg'					=> '社名',
			'staff_company_name'		=> '社名',
			'staff_company_name_kana'	=> '社名（フリガナ）',
			'staff_zip'					=> '郵便番号',
			'staff_pref'				=> '都道府県',
			'staff_addr1'				=> '市区群',
			'staff_addr2'				=> '町名番地',
			'staff_addr3'				=> 'ビル名',
			'staff_addr_kana'			=> '住所（フリガナ）',
			'staff_name_sei'			=> 'ご担当者名：姓',
			'staff_name_kana_sei'		=> 'ご担当者名（フリガナ）：セイ',
			'staff_name_mei'			=> 'ご担当者名：名',
			'staff_name_kana_mei'		=> 'ご担当者名（フリガナ）：メイ',
			'staff_section'				=> '部署／役職',
//			'staff_post'				=> '役職',
			'staff_mail'				=> 'メールアドレス',
			'staff_tel'					=> '連絡用電話番号',
...
// STEP2
			'check-invoice'				=> '',
			'invoice_company_name_fg'	=> '',
			'invoice_company_name'		=> '社名',
			'invoice_company_name_kana'	=> '社名（フリガナ）',
			'invoice_addr_fg'			=> '',
			'invoice_zip'				=> '郵便番号',
			'invoice_pref'				=> '都道府県',
			'invoice_addr1'				=> '市区群',
			'invoice_addr2'				=> '町名番地',
			'invoice_addr3'				=> 'ビル名',
			'invoice_addr_kana'			=> '住所（フリガナ）',
			'invoice_name_sei'			=> 'ご担当者名：姓',
			'invoice_name_mei'			=> 'ご担当者名：名',
			'invoice_name_kana_sei'		=> 'ご担当者名（フリガナ）：セイ',
			'invoice_name_kana_mei'		=> 'ご担当者名（フリガナ）：メイ',
			'invoice_section'			=> '部署',
			'invoice_post'				=> '',
			'invoice_tel'				=> '連絡用電話番号',
			'invoice_fax'				=> '連絡用FAX番号',

			'fin_name'					=> '金融機関名',
			'fin_branch_name'			=> '支店名',
			'fin_account_type'			=> '口座種別',
			'fin_account_number'		=> '口座番号',
			'fin_account_name'			=> '口座名義',
			'fin_account_name_kana'		=> '口座名義（フリガナ）',
...
// STEP3
			...
			'goods_price3'				=> '③予定販売価格',
			'goods_image3'				=> '③商品画像',
			'price_range_min'			=> '商品価格帯（目安）最小値',
			'price_range_max'			=> '商品価格帯（目安）最大値',
			'other_site_url'			=> '他サイトURL',

			'distributor'				=> '販売事業者',
			'corp_name'					=> 'ショップ名（15文字以内）',
			'corp_name_kana'			=> 'ショップ名（フリガナ）',
			'corp_name_en'				=> '英字表記',
			'location_fg'				=> '所在地',
			'supervisor_zip'			=> '郵便番号',
			'supervisor_pref'			=> '都道府県',
			'supervisor_addr'			=> '市区群',
			'supervisor_addr2'			=> '町名番地',
			'supervisor_addr3'			=> 'ビル名',
			'supervisor_addr_kana'		=> '住所（フリガナ）',
			'supervisor_fg'				=> '',
			'supervisor_name_sei'		=> '運営統括責任者：姓',
			'supervisor_name_mei'		=> '運営統括責任者：名',
			'supervisor_mail'			=> 'メールアドレス',
			'supervisor_tel'			=> '電話番号',
			'supervisor_fax'			=> 'FAX番号',
			'contact_s_time'			=> '問合せ受付可能営業時間１',
			'contact_e_time'			=> '問合せ受付可能営業時間２',
			'expenses'					=> '商品以外の必要代金',
			'expenses_other'			=> '商品以外の必要代金「その他」',
			'defective'					=> '不良品の取扱',
			'defective_other'			=> '不良品の取扱「その他」',
			...
			'sales_qty_other'			=> '販売数量「その他」',
			'delivery_time'				=> '引渡し時期',
			'delivery_time_none'		=> '引渡し時期',
			'payment'					=> '支払方法',
			'payment_other'				=> '支払方法「その他」',
			'due_payment'				=> '支払期限',
			'about_returns'				=> '返品について',
			'about_returns_other'		=> '返品について「その他」',
			'due_returns'				=> '返品期限',
